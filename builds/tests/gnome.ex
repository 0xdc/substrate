...
spawn virt-install --autoconsole text --os-variant gentoo --boot uefi --location $CD,kernel=boot/gentoo,initrd=boot/gentoo.igz --extra-args "console=ttyS0 cdroot verify quiet" --metadata title=$CD {*}$EXTRA
...
while true {
	expect {
		...
		"root@root-x86-64 ~ #" {
			...
			if {$LIVE == 9} { send "systemctl enable NetworkManager.service gdm.service\r" }
